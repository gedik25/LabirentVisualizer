...
void Renderer::clear() {
    m_window.clear(getTheme().background);
}
...
void Renderer::renderCellWalls(int x, int y) {
    ...
    if (hasFlag(cell, CellFlags::WallNorth)) {
        ...
        m_wallShape.setPosition({px - m_wallThickness / 2, py - m_wallThickness / 2});
    ...
    if (hasFlag(cell, CellFlags::WallEast)) {
        ...
        m_wallShape.setPosition({px + m_cellSize - m_wallThickness / 2, py - m_wallThickness / 2});
        m_window.draw(m_wallShape);
    }
    ...
    if (hasFlag(cell, CellFlags::WallSouth)) {
        m_wallShape.setSize({m_cellSize + m_wallThickness, m_wallThickness});
        m_wallShape.setPosition({px - m_wallThickness / 2, py + m_cellSize - m_wallThickness / 2});
        m_window.draw(m_wallShape);
    }
