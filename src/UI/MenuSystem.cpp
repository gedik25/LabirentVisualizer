...
MenuSystem::MenuSystem() {
  ...
  m_sizePresets = {{"Small (25x25)", 25, 25},
                   {"Medium (50x50)", 50, 50},
...
bool MenuSystem::init(sf::RenderWindow &window) {
  ...
  solveSection.items = {{"Algorithm",
                         ItemType::Selector,
                         {"BFS", "DFS", "A* (A-Star)", "Dijkstra",
                          "Greedy Best-First", "Bidirectional BFS", "JPS"},
                         m_selectedSolver,
                         nullptr,
                         [this](int idx) {
  ...
  sizeSection.items = {{"Size", ItemType::Selector, sizeOptions,
                        m_currentSizePreset, nullptr, [this](int idx) {
                          m_currentSizePreset = idx;
                          m_gridWidth = m_sizePresets[idx].width;
                          m_gridHeight = m_sizePresets[idx].height;
                          if (onSizeChange)
                            onSizeChange(m_gridWidth, m_gridHeight);
                        }}};
      ...
      {"Level",
       ...
       [this](int idx) {
         ...
         if (onDifficultyChange)
           onDifficultyChange(m_difficulty);
       }}};
...
void MenuSystem::hide() { m_visible = false; }
...
void MenuSystem::selectItem() {
  if (m_currentSection >= static_cast<int>(m_sections.size()))
    return;

  auto &section = m_sections[m_currentSection];
  if (m_currentItem >= static_cast<int>(section.items.size()))
    return;
...
void MenuSystem::cycleOption(int direction) {
  ...
  if (item.type == ItemType::Selector && !item.options.empty()) {
    ...
    item.selectedOption =
        (item.selectedOption + direction + numOptions) % numOptions;
...
void MenuSystem::renderSection(sf::RenderWindow &window,
                               const MenuSection &section, float &y,
                               int sectionIdx) {
  sf::Vector2u windowSize = window.getSize();
  float menuX = (windowSize.x - m_menuWidth) / 2.0f;

  // Section title
  renderText(window, section.title, menuX + m_padding, y, m_titleColor,
             m_fontSize);
  y += m_fontSize + 5.0f;
  ...
  for (size_t i = 0; i < section.items.size(); ++i) {
    ...
    if (isSelected) {
      sf::RectangleShape selector(
          {m_menuWidth - 2 * m_padding, m_itemHeight - 4});
