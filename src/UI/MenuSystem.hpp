...
public:
    ...
    void setGridSize(int width, int height) { m_gridWidth = width; m_gridHeight = height; }
...
private:
    ...
    // Current selections
    int m_selectedGenerator = 0;  // 0=RecursiveBacktracking, 1=Prim, 2=Kruskal, 3=BinaryTree, 4=Ellers
    int m_selectedSolver = 0;     // 0=BFS, 1=DFS, 2=A*, 3=Dijkstra, 4=Greedy, 5=Bidirectional
    ...
    void renderText(sf::RenderWindow& window, const std::string& text, 
                    float x, float y, const sf::Color& color, unsigned int size = 0);
